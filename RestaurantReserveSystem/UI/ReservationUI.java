	...
	public static void displayReservationUI() {
		...
		do {
			...
			for (int i = 0; i < tableArr.size(); i++) {
				if (tableArr.get(i).getMaxNumSeats() >= pax && tableArr.get(i).getAvailStatus() == false) {
					System.out.println("Enter customer contact number: \n");
					String number = sc.nextLine();
					///////////////////////////////////////////////////////////////////////////////////////////////
					Reservation r = new Reservation(date, time, pax, name, number, tableArr);
					Manager.ReservationMgr.addReservationBooking(r);
		...
		for (int i = 0; i < tableArr.length; i++) {
			if (tableArr[i].tableReservation != null) {
				...
				System.out.printf("Reservation at %s, %s for %d pax.\nTable number: %d\nCreator: %s\nNumber: %s\n\n",
						r.reserveTime.toString(), r.reserveDate.toString(), r.numOfPax, r.tab.tableId, r.getName(),
						r.getNum());
