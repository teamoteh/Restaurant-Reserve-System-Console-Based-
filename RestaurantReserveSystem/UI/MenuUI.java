...
public class MenuUI {
	...
	* sc.nextLine() is used instead of sc.next(), as sc.next() stops reading at whitespaces
	* Hence when inputting foodDesc e.g. "Spicy Chicken Burger" sc.next() will be equal to Spicy only
	* therefore had to use sc.nextLine()
	* Integer and Double are being parsed, to allow proper execution of sc.nextLine()
	* Without parse THE PROBLEM will occur.
	* THE PROBLEM: https://stackoverflow.com/questions/13102045/scanner-is-skipping-nextline-after-using-next-or-nextfoo
	* Menu should be working fine now, only need to figure out how to write to text file.
	* To run it remember to initialise it in Restaurant App at the start
	* Initialise command: MenuMgr.getMenu();
	*
	* For the txt files the commas after the last item were the reason why MenuMgr got errors
	*
		...
		do {
			...
			switch(user_choice) {
				...
				case 6:
					...
					//MenuMgr.addToMenu(promoName, promoPrice, promoDesc); Supposed to be Promo Manager
					break;
